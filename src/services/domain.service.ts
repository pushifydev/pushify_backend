...
export const domainService = {
  ...
  async createAutoSubdomain(projectId: string, projectSlug: string, serverId: string) {
    ...
    const subdomainName = `${projectSlug}.${previewBaseUrl}`;
    ...
    // Also check if the domain name is already taken globally
    const existingDomain = await domainRepository.findByDomain(subdomainName);
    if (existingDomain) {
      logger.warn({ projectId, domain: subdomainName }, 'Auto subdomain domain name already taken globally');
      return null;
    }
  ...
  async verify(
    ...
    const isAutoSubdomain = domain.isAutoGenerated;
    ...
    // DNS is correct (or auto subdomain), now configure Nginx and SSL
    ...
    try {
      ...
      if (isAutoSubdomain) {
        logger.info({ domain: domain.domain, containerPort }, 'Configuring Nginx for auto subdomain with wildcard cert');
      ...
      // Configure Nginx for HTTP first (custom domains)
