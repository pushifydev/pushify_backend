...
export const rustBuildpack: Buildpack = {
  ...
  generateDockerfile(config: BuildpackConfig): string {
    ...
    // A custom build command runs instead of `cargo build --release`; the first executable in
    // target/release still becomes /server, and a custom start command replaces it entirely.
    const build = config.buildCommand || 'cargo build --release';
