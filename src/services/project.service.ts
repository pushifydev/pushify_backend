...
export const projectService = {
  ...
  async delete(
    ...
    // DNS records of its auto subdomains (and PR previews') — see lib/cloudflare-dns.ts
    try {
      const { domains: domainsTable } = await import('../db/schema');
      ...
      const autoDomains = await db
        ...
        .from(domainsTable)
        .where(and(eq(domainsTable.projectId, projectId), eq(domainsTable.isAutoGenerated, true)));
      const previewHosts = (await previewRepository.findAllByProject(projectId, 500))
        .map((preview) => hostnameOf(preview.previewUrl))
        .filter((host): host is string => !!host);
      for (const name of [...autoDomains.map((d) => d.domain), ...previewHosts]) {
        await deleteAutoSubdomainRecordQuietly(name);
      }
    } catch (err) {
      logger.warn({ projectId, err }, 'Failed to remove auto subdomain DNS records');
    }
