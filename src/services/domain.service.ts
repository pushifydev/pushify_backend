...
import { RESERVED_SUBDOMAIN_LABELS, deleteAutoSubdomainRecordQuietly } from '../lib/cloudflare-dns';
...
export const domainService = {
  ...
  async createAutoSubdomain(
    ...
    // Check if this auto-generated subdomain already exists for this environment
    const existing = (await domainRepository.findByProject(projectId)).find(
      (row) => row.isAutoGenerated && (row.environment ?? 'production') === environment
    );
    if (existing) {
    ...
    // Generate unique subdomain — add short suffix if slug is taken, or is a name Pushify itself
    // uses (a project called "api" must not get api.pushify.dev)
    let subdomainName = `${projectSlug}.${previewBaseUrl}`;
    const existingDomain = RESERVED_SUBDOMAIN_LABELS.has(projectSlug) || (await domainRepository.findByDomain(subdomainName));
    if (existingDomain) {
    ...
    // If this environment already has domains, this auto subdomain should not be primary
    const existingDomains = (await domainRepository.findByProject(projectId)).filter(
      (row) => (row.environment ?? 'production') === environment
    );
  ...
  async delete(
    ...
    if (domain.isAutoGenerated) await deleteAutoSubdomainRecordQuietly(domain.domain);
