...
import { gradleDepsRun, mavenDepsRun } from '../lib/platform-docker';
import type { Buildpack, BuildpackDetectResult, BuildpackConfig } from './types';

type JavaCommands = { build?: string | null; start?: string | null };
...
export const javaBuildpack: Buildpack = {
  ...
  generateDockerfile(config: BuildpackConfig): string {
    ...
    const cmds = { build: config.buildCommand, start: config.startCommand };
    if (isGradle) return this._gradle(copyPrefix, rootDir, port, cmds);
    return this._maven(copyPrefix, rootDir, port, cmds);
  },
  ...
  _maven(copyPrefix: string, rootDir: string, port: number, cmds: JavaCommands): string {
    return `# Generated by Pushify — Maven/Spring on glibc Linux
...
RUN ${cmds.build || 'mvn package -DskipTests -q'}
...
${cmds.start ? shCmd(cmds.start) : 'CMD ["sh", "-c", "java $JAVA_OPTS -jar app.jar --server.port=$PORT"]'}
`;
  },

  _gradle(copyPrefix: string, rootDir: string, port: number, cmds: JavaCommands): string {
    return `# Generated by Pushify — Gradle on glibc Linux
...
COPY ${rootDir === '.' ? '.' : rootDir} .
RUN ${cmds.build || 'gradle build -x test --no-daemon -q'}
